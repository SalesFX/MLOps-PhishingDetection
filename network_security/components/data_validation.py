    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if not self.validate_numerical_columns_exist(train_dataframe):
                raise ValueError(
                    "Train dataframe is missing required numerical columns or has wrong types."
                )

            if not self.validate_numerical_columns_exist(test_dataframe):
                raise ValueError(
                    "Test dataframe is missing required numerical columns or has wrong types."
                )
            ...
            if not drift_status:
                raise ValueError("Dataset drift detected between train and test sets.")
